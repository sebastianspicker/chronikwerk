...
def test_nfr4_settings_dict_redacts_secret_keys() -> None:
    ...
    raw = {
        "api_token": fake_credential("secret123"),
        "nested": {"webhook_hmac_secret": fake_credential("hmac456")},
    }
    out = redact_settings_dict(raw)
    check(not not out["api_token"] == REDACTED_VALUE, "assertion failed")
    check(not not out["nested"]["webhook_hmac_secret"] == REDACTED_VALUE, "assertion failed")
    check(not not raw["api_token"] == "secret123", "assertion failed")
...
def test_nfr4_scrub_redacts_token_like_patterns_in_text() -> None:
    ...
    check(not not "leak123" not in out, "assertion failed")
    check(not not "xyz789" not in out, "assertion failed")
