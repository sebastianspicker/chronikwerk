...
```bash
ARCHIVER_ENV_FILE=.env.example STORAGE__ROOT=/tmp/zammad-archive docker compose up -d --build
docker inspect --format '{{.State.Health.Status}}' "$(docker compose ps -q zammad-pdf-archiver)"
ARCHIVER_ENV_FILE=.env.example STORAGE__ROOT=/tmp/zammad-archive docker compose down --remove-orphans
```
...
- `/metrics` exposure:
  - If metrics are enabled (`OBSERVABILITY__METRICS_ENABLED=true`), verify `GET /metrics` is reachable only from approved internal sources (reverse proxy ACL, firewall, or private network).
  - Verify unauthorised external access is blocked.
- CIFS/SMB write safety:
  - Confirm the service identity (UID/GID `10001` by default in container image) can create directories and files under `STORAGE__ROOT`.
  - Confirm `STORAGE__ROOT` is mounted read-write and has sufficient free space/quota.
