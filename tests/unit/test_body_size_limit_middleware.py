"""Exercises body-size middleware limits and request-drain behavior."""
...
from chronikwerk.app.middleware.body_size_limit import (
...
async def _inner_app(scope: Any, receive: Any, send: Any) -> None:
    """Record inner-app execution without adding an unrelated response body."""
...
def _make_scope(path: str = "/ingest", content_length: int | None = None) -> dict[str, Any]:
    """Build the scope fixture used by this scenario."""
    headers = []
...
def test_middleware_times_out_a_stalled_stream(tmp_path) -> None:
    settings = make_settings(
        str(tmp_path),
        overrides={"hardening": {"body_size_limit": {"max_bytes": 100, "timeout_seconds": 0.01}}},
    )
    responses: list[Any] = []
...
def test_middleware_caps_chunked_unauthenticated_admin_body(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        "chronikwerk.app.middleware.body_size_limit._ADMIN_BODY_MAX_BYTES",
        10,
    )
    settings = make_settings(
        str(tmp_path),
...
def test_disabled_configured_limit_still_uses_absolute_safety_cap(
    ...
    monkeypatch.setattr(
        "chronikwerk.app.middleware.body_size_limit._ABSOLUTE_INGEST_MAX_BYTES",
        10,
    )
    settings = make_settings(
        str(tmp_path),
