"""Verifies RFC 3161 response validation and TSA transport safety."""
...
import chronikwerk.adapters.signing.tsa_rfc3161 as tsa_module
...
from chronikwerk.adapters.signing.tsa_rfc3161 import build_timestamper  # noqa: E402


def _tsa_req() -> Any:
    """Build a deterministic RFC 3161 request for response-validation cases."""
...
def _make_signing(tsa_url: str) -> SigningSettings:
    """Build a signing service configured for the test scenario."""
...
def _capturing_async_client(captured: dict[str, Any], captured_kwarg: str):
    """Return an HTTPX client double that records a transport constructor option."""

    class _CapturingAsyncClient:
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            captured[captured_kwarg] = kwargs.get(captured_kwarg)
...
def test_tsa_connects_to_the_validated_address_with_original_host_identity(
    ...
    monkeypatch.setattr(
        "chronikwerk.config.transport.socket.getaddrinfo",
    ...
    with respx.mock:
        route = respx.post("https://93.184.216.34/rfc3161").mock(return_value=httpx.Response(500))
...
def test_tsa_http_client_respects_transport_trust_env(
    ...
    tsa_url = "https://tsa.test/rfc3161"
    signing = _make_signing(tsa_url)
    timestamper = build_timestamper(signing, trust_env=trust_env, allow_private_networks=True)

    captured: dict[str, Any] = {}

    monkeypatch.setattr(httpx, "AsyncClient", _capturing_async_client(captured, "trust_env"))
...
def _make_signing_with_auth(user: str | None, password: str | None) -> SigningSettings:
    """Build the signing with auth fixture used by this scenario."""
    return SigningSettings(
        enabled=True,
        pfx_path="/private/tmp/test-signing.pfx",  # type: ignore[arg-type]
        timestamp=SigningTimestampSettings(
...
def test_tsa_ca_bundle_path_is_used_as_verify(
    ...
    from chronikwerk.adapters.signing.tsa_rfc3161 import (
    ...
    monkeypatch.setattr(httpx, "AsyncClient", _capturing_async_client(captured, "verify"))
...
def test_tsa_response_declared_over_limit_is_rejected_before_body_read(
    ...
    timestamper = build_timestamper(_make_signing(tsa_url), allow_private_networks=True)
