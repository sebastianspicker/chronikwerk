"""Exercises ingest validation, request metadata, idempotency, and retry authorization."""
...
from chronikwerk.app.constants import FORCE_REPROCESS_KEY
...
def _signed_headers(body: bytes, headers: dict[str, str] | None = None) -> dict[str, str]:
    """Build authenticated webhook headers for a test payload."""
...
def _test_settings_require_delivery_id(storage_root: str) -> Settings:
    """Build ingest settings that reject requests without a delivery identifier."""
...
def _post_ingest_with_request_id(tmp_path, monkeypatch, request_id: str):
    """Submit an ingest request while isolating request-ID middleware behavior."""
...
def test_request_id_header_is_preserved(tmp_path, monkeypatch) -> None:
    response = _post_ingest_with_request_id(tmp_path, monkeypatch, "test-req-id")
    assert response.status_code == 202
...
def test_request_id_header_invalid_value_is_replaced(tmp_path, monkeypatch) -> None:
    response = _post_ingest_with_request_id(tmp_path, monkeypatch, "bad value with spaces")
    assert response.status_code == 202
...
def _test_settings_with_retry_token(storage_root: str, **extra_overrides: Any) -> Settings:
    """Build ingest settings that authorize explicit retry operations."""
