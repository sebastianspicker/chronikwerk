...
def _capture_admin_flow(page: Any, args: argparse.Namespace, out_dir: Path) -> None:
...
def _capture_dlq_flow(page: Any, out_dir: Path) -> None:
...
def _capture_unauthorized_history(desktop: Any, base_url: str, out_dir: Path) -> None:
...
def _capture_backend_unavailable(page: Any, args: argparse.Namespace, out_dir: Path) -> None:
...
def _restart_redis_demo(args: argparse.Namespace) -> None:
...
def _capture_mobile_view(browser: Any, args: argparse.Namespace, out_dir: Path) -> None:
...
def _capture(args: argparse.Namespace) -> int:
    sync_playwright, Error, PlaywrightTimeoutError = _import_playwright()
    ...
    with sync_playwright() as p:
        ...
        try:
            _capture_admin_flow(page, args, out_dir)
            _capture_dlq_flow(page, out_dir)
            _capture_unauthorized_history(desktop, args.base_url, out_dir)
            _capture_backend_unavailable(page, args, out_dir)
            redis_was_stopped = True

            _restart_redis_demo(args)
            redis_was_stopped = False
            _capture_mobile_view(browser, args, out_dir)
